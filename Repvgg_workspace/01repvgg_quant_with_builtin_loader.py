...
def evaluate_imageNet(model, loader, config, save_dir=".", conf_thres=0.001, iou_thres=0.65):
    if save_dir and os.path.dirname(save_dir) != "": # 如果路径不存在或父目录为空
        os.makedirs(os.path.dirname(save_dir), exist_ok=True)

    model = deepcopy(model)
    acc1_meter_avg, _, _ = validate(config, loader, model)
    return acc1_meter_avg# 这里要返回acc值



def pipeline(args, config):
    # print("0.0 change Conv2d to QuantConv2d......")
    # torch.nn.Conv2d = sx_quant.QuantConv2d
    ...
    dataset_train, dataset_val, train_dataloader, val_dataloader, mixup_fn = build_loader(config)

    
    print("3.0 Begining Calibration ....")
    Linker(model).do_collect = True
    start1 = time.time()
    collect_stats(model, train_dataloader, "cuda")
    ...
    summary = sx_quant.SummaryTool(args.summary)

    Linker(model).do_quant = True
    # 开启量化后，关闭yolov5最后一个detect层。
    # Linker(model.stage0.rbr_dense).do_quant = False
    # Linker(model.stage0.rbr_1x1).do_quant = False
    ap = evaluate_imageNet(model, val_dataloader, config)
    summary.append(["sx_quant", ap])
    Linker(model).do_quant = False

    print("6.0 ori infer ....")
    ap = evaluate_imageNet(model,val_dataloader, config)
    summary.append(["ori", ap])
    if True:
        print("Export PTQ...")
        ...
        model.float()
        input_dummy = torch.zeros(1, 3, 224, 224, device=device)
        model.eval()
        with torch.no_grad():
            torch.onnx.export(model, input_dummy, "repvgg_ptq.onnx", opset_version=14)
...
if __name__ == "__main__":
    ...
    # parser = argparse.ArgumentParser(description='YOLOV5 quantization flow script')
    # parser.add_argument('--weight', type=str, default='yolov5s.pt', help='initial weights path')
    # parser.add_argument('--cocodir', type=str, default='/dataset/coco2017', help='coco dataset directory')
    ...
    # parser.add_argument('--calib_batch_size', type=int, default=200, help='calib batch size for collect,default=200')
    ...
    # parser.add_argument("--ptq", type=str, default="yolov5s_sx_quant.onnx", help="file")
    # pipeline(**parser.parse_args().__dict__)
    if config.AMP_OPT_LEVEL != "O0":
