...
def prepare_val_dataset(cocodir, batch_size=4):
    dataloader = create_dataloader(
        ...
        augment=False, hyp=None, rect=False, cache=False, stride=32,pad=0.5, image_weights=False, shuffle=True
    )[0]
    return dataloader
...
def prepare_train_dataset(cocodir, batch_size=4):
    ...
    dataloader = create_dataloader(
        ...
        imgsz=640,
        batch_size=batch_size,
        augment=True, hyp=hyp, rect=False, cache=False, stride=32,pad=0, image_weights=False, shuffle=True
    )[0]
    return dataloader
...
def calibrate_model(model, dataloader, device):
    ...
    print(f"collect cost {time.time() - start} s")
    # 获取动态范围,计算amax值,scale值
    print("start compute amax。。。")
    start = time.time()
    compute_amax(model, device, method='mse')
    print(f"compute amax cost {time.time() - start} s")
